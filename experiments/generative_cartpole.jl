...
function get_policy_thresholded(init_latent, context, threshold_val=10)
    latent = param(copy(init_latent))
    latents = []
    Rs = []
    R = 0
    best_score = 100000000f0 # just a large initial number
    above_thresh = false
    while above_thresh == false
        R = reward_loss(context, latent, [0,0])
        Rdata = R.data
        if Rdata <= best_score
            best_score = Rdata
        end
        if Rdata >= best_score + (best_score / threshold_val)
            above_thresh = true
            break
        end
        Flux.back!(R, Rdata)
